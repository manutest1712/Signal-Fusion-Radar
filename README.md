...
**Comparison Rule:** If the CUT power ($P_{\text{CUT}}$) from the Range-Doppler Map (RDM) is greater than the Threshold ($T_h$), the cell is declared a target. The cell is marked as a target (`1`) in the final CFAR output map ($M_{\text{CFAR}}$).
